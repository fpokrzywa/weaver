...
import { ChevronRight, ChevronDown, Play, Search, Menu } from 'lucide-react';
...
const RightPanel: React.FC<RightPanelProps> = ({ isExpanded = false, isFullScreen = false, onExpandAll }) => {
  ...
  return (
    <div className={`${isExpanded ? 'flex-1' : 'w-1/2'} bg-white border-l border-gray-200 h-screen flex flex-col`}>
      {/* Full Screen Header - only show when in full screen mode */}
      ...
      {/* Moveworks Branding */}
      <div className={`${isExpanded ? 'p-16' : 'p-8'} text-center ${isFullScreen ? 'pt-8' : ''}`}>
        <div className="mb-6">
          <img 
